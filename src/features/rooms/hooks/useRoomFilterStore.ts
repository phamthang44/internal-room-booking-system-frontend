...
import type { RoomSortApi } from "../types/classroom.api.types";
...
const DEFAULT_FILTER: Pick<
  ...
  | "minCapacity" | "maxCapacity" | "equipmentId" | "sort"
  | "viewMode"
> = {
  search: "",
  bookingDate: "",
...
export const useRoomFilterStore = create<RoomFilterState>((set, get) => ({
  ...
  activeFilterCount: () => {
    const { bookingDate, timeSlotId, minCapacity, maxCapacity, equipmentId, sort } = get();
    let count = 0;
